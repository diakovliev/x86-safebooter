...
LDARGS			= -A i386 -melf_i386 -telf_i386 
...
define LD_LIB_CMD 
	...
	$(LD) $(LDARGS) --whole-archive -r -Map=$@.map $1 -o$@.lib $(SUPPRESS_LD_OUTPUT) && \
	$(GREEN) && printf "OK\n" && $(NORMAL)
endef

define LD_IMG_CMD 
	$(CYAN) && printf " [LD]" && $(NORMAL) && printf " %s " $@ && \
	$(LD) $(LDARGS) -N -static -Ttext $2 -Map=$@.map $1 -o$@.elf $(SUPPRESS_LD_OUTPUT) && \
